...
} from '../common/search-in-workspace-service';
import {
...
class QuickSearchInWorkspaceResultItem extends QuickOpenItem {
        ...
        constructor(result: SearchInWorkspaceResult, openerService: OpenerService, labelProvider: LabelProvider) {
                ...
                const file = labelProvider.getLongName(new URI('file://' + result.file));

                const opts: QuickOpenItemOptions = {
                        detail: result.lineText,
                        detailHighlights: [hl],
                        label: `${file}:${result.line}:${result.character}`,
                };
                super(opts);
...
@injectable()
export class QuickSearchInWorkspaceKeybindingContribution implements KeybindingContribution {
